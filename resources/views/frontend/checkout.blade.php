                    ...
                    <div class="col2-set" id="customer_details">
                        ...
                        <div class="woocommerce-billing-fields">
                            ...
                            <p class="form-row form-row form-row-first validate-required" id="billing_first_name_field">
                                ...
                                <input type="text" class="input-text " value="{{Auth::guard('customer')->user()->name}}" name="name" id="billing_first_name" placeholder="" autocomplete="given-name" value="" />
                            </p>
                            <p class="form-row form-row form-row-last validate-required validate-email" id="billing_email_field">
                                <label for="billing_email" class="">Email Address <abbr class="required" title="required">*</abbr></label>
                                <input type="email" class="input-text " value="{{Auth::guard('customer')->user()->email}}" name="email" id="email" placeholder="" autocomplete="email" value="" />
                            </p>
                            <div class="clear"></div>
                        ...
                        <table class="shop_table woocommerce-checkout-review-order-table">
                            <tr class="cart-subtotal">
                                ...
                                <td>
                                    ...
                                    <span class="woocommerce-Price-amount amount sub_total">{{$sub_total}}</span>
                                </td>
                            </tr>
                            <tr class="cart-subtotal">
                                <th>Discount</th>
                                <td>
                                    ...
                                    <span class="woocommerce-Price-amount amount discount">{{ session('discount') }}</span>
                                </td>
                            </tr>
                            <tr class="shipping">
                                <th>Delivery Charge</th>
                                <td data-title="Shipping">
                                <span class="woocommerce-Price-currencySymbol charge_show">0</span>
                            ...
                            <tr>                            
                                <td>
                                    ...
                                    <label for="outside_city">Outside City</label>
                                </td>
                            </tr>
                            <tr class="order-total">
                                <th>Total</th>
                                <td><strong><span class="woocommerce-Price-amount amount grand_total">{{ $sub_total - ($sub_total * (session('discount'))) / 100 }}</span></strong> </td>
                            </tr>
                        </table>
...
<script>
    $('.input_delivery_charge').click(function() {
        var input_charge = $(this).val();
        var sub_total = $('.sub_total').text();
        var show_charge = $('.charge_show').html(input_charge);
        var discount = $('.discount').text();
        // var grand_total = parseInt(sub_total)+parseInt(input_charge);
        var after_cal = parseInt(sub_total) - (sub_total * discount) / 100;
        // alert(after_cal);

        $('.grand_total').html(after_cal+parseInt(input_charge));
        // alert(discount);
    });
</script>
@endsection